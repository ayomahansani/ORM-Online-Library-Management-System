...
public class BranchManageFormController {
    ...
    private boolean validateBranchInfo(){

        //validate id
        String id = txtBranchId.getText();
        boolean matchesId = Pattern.matches("(Br)[0-9]{3,}", id);
        if(!matchesId){
    ...
    void btnAddOnAction(ActionEvent event) {
        ...
        boolean isValidated = validateBranchInfo();
    ...
    void txtBranchSearchOnAction(ActionEvent event) {
        ...
        if ((Pattern.matches("[A-Za-z]{2,}[^!@%* .\\d]", txtBranchSearch.getText()))) {
